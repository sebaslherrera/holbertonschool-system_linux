...
void read_content(char *flags, DIR *dir, DListElmt *dir_head,
				  ...
				  DList *fd_of_dirs, DList *errors, int *exit_co)
{
	struct dirent *read;
	...
	else
	{
		...
		if ((n_files > 0 && n_directories > 0) || n_directories > 1 || *exit_co > 0)
		{
 ...
 * @errors: Array of arguments - Array of strings
 * @exit_code: Array of arguments - Array of strings
 * Return: 0 on success, status codes on failure
 */
void iterate_directories(DList *directories, DList *fd_of_dirs, int n_files,
						 int n_directories, char *flags, DList *errors, int *exit_code)
{
	...
	while (dir_head != NULL)
	{
		...
		read_content(flags, dir, dir_head, n_files,
					 n_directories, fd_of_dirs, errors, exit_code);
